var FORMS_API_FIELD = {
	...
	t_formsApi__initTildaField = function t(e) {
		return new Promise(function (t) {
			...
			return r
				? t(r)
				: t_formsApi__getFieldConstructor(e).onFieldReady().then(t);
		});
	};
...
function t_forms__removePlaceholder(t) {
	...
	if ("INPUT" === e.tagName) {
		...
		o &&
			n &&
			((window.t_forms__inputData[n] = o), e.setAttribute("placeholder", ""));
	}
}
...
function t_forms__addPlaceholder(t) {
	...
	a &&
		o &&
		(e.setAttribute("placeholder", a), (window.t_forms__inputData[o] = ""));
}
			...
			if (
				...
				Array.prototype.forEach.call(o, function (t) {
					...
					e &&
						...
						r.classList.contains("t-form__inputsbox_inrow") ||
							t_forms__calculateFieldsWidthInJS(t, n);
				});
		...
		if (o) {
			...
			if (
				...
				if (
					...
					var c = t_forms__getErrorContainers(o, ""),
						m = c.errorBoxes,
						f = c.allError;
					return (
				...
				if (o.querySelector(".g-recaptcha") && grecaptcha) {
					...
					if (_) grecaptcha.reset(_);
						...
						(_ = grecaptcha.render(n + "recaptcha", p)),
							(o.tildaCaptchaClientId = _);
					}
...
(window.t_forms__lang = (
		...
		for (
			...
			if (!i(l)) {
				var d = !!parseInt(l.getAttribute("data-tilda-req") || 0, 10),
					u = l.getAttribute("data-tilda-rule") || "none",
					c = "",
					m = "",
					f = l.getAttribute("data-tilda-rule-minlength") || 0,
					_ = l.getAttribute("data-tilda-rule-maxlength") || 0,
					p = {},
					w = l.value,
					g = "",
					y = l.getAttribute("type"),
					h = l.getAttribute("name"),
				...
				(p.obj = l),
					...
					f && (f = parseInt(f, 10)),
					_ && (_ = parseInt(_, 10));
				var b = !w.length && !g.length,
					E = "checkbox" === y || "radio" === y,
					F = e.querySelectorAll('[name="' + h + '"]:checked').length;
				if ((((w.length && !E) || (E && F)) && (n = !1), d && (b || (E && !F))))
					p.type.push("req");
				else {
					switch (u) {
						case "email":
							(c =
								...
								w.length && !w.match(c) && p.type.push("email");
							break;
						case "url":
							(c =
								...
								w.length &&
									...
									m.length &&
									m[0]
										? (m = m[0]).match(c) || p.type.push("url")
										: ((m && !m[0]) || p.type.push("url"), (m = "")));
							break;
						case "phone":
							var A = l.getAttribute("data-tilda-mask"),
								x = "^[0-9()+-";
							A &&
								(-1 !== A.indexOf("*")
									? (x += "a-zёа-я")
									: (-1 !== A.indexOf("a") && (x += "a-z"),
									  -1 !== A.indexOf("а") && (x += "а-яё"))),
								(x += "]+$"),
								(c = new RegExp(x, "gi")),
								((g.length && !g.match(c)) ||
									1 == (m = g.replace(/[^0-9]+/g, "")).indexOf("000") ||
						...
						case "number":
							(c = /^[0-9]+$/gi),
								g.length > 0 && !g.match(c) && p.type.push("number");
							break;
						case "date":
						...
						case "name":
							(c =
								...
								w.length && !w.match(c) && p.type.push("name");
							break;
						case "nameeng":
							(c = /^([A-Za-z\s]{1,}((\-)?[A-Za-z\.\s](\')?){0,})*$/i),
								w.length && !w.match(c) && p.type.push("nameeng");
							break;
						case "namerus":
							(c = /^([А-Яа-яЁё\s]{1,}((\-)?[А-Яа-яЁё\.\s](\')?){0,})*$/i),
								w.length && !w.match(c) && p.type.push("namerus");
							break;
						case "string":
							(c =
								...
								w.length && !w.match(c) && p.type.push("string");
							break;
						case "chosevalue":
							var S;
							"true" === l.getAttribute("data-option-selected") ||
								p.type.push("chosevalue");
							break;
						case "promocode":
							"y" !== v ||
								!g.length ||
					...
					f > 0 && w.length && w.length < f && p.type.push("minlength"),
						_ > 0 && w.length && w.length > _ && p.type.push("maxlength");
				}
				p.type && p.type.length && (o[o.length] = p);
			}
		}
		if ("y" === v) {
			var C = window.tcart_minorder > 0,
				I = window.tcart_mincntorder > 0,
				k,
				p,
				p;
			if (C)
				if (
					...
					(p = { obj: {}, type: [] }).type.push("minorder"), o.push(p);
			if (I && window.tcart.total < window.tcart_mincntorder)
				(p = { obj: {}, type: [] }).type.push("minquantity"), o.push(p);
		}
		return (
			n &&
				!o.length &&
	...
	(window.tildaForm.showErrorInPopup = function (t, e) {
		...
		for (var f = 0; f < e.length; f++)
			if (e[f] && e[f].obj) {
				...
				_ && (a = _.closest(n)),
					a &&
						((d = a.querySelectorAll(".t-input-error")),
				...
				for (var p = 0; p < e[f].type.length; p++) {
					...
					(u = ""),
						(l = r.querySelector(".js-rule-error-" + w))
							? (l.textContent && l.innerText) || !g || -1 !== c.indexOf(g)
								? ((u = l.textContent || l.innerText),
								  -1 === c.indexOf(u) &&
										(c = c + '<p class="t-form__errorbox-item">' + u + "</p>"))
								: (c = c + '<p class="t-form__errorbox-item">' + g + "</p>")
							: g &&
							  -1 === c.indexOf(g) &&
							  (c = c + '<p class="t-form__errorbox-item">' + g + "</p>"),
						i &&
							(!u && t_forms__getMsg(w + "field")
								? (u = t_forms__getMsg(w + "field"))
								: g && (u = g),
							u &&
								a &&
		...
		if (c) {
			...
			if (
				...
				if (h && h.length) {
					var v,
						b = '.t1093 .t-popup[data-tooltip-hook="' + h[h.length - 1] + '"]',
		...
		if (
			...
			document.addEventListener || (C = "focusin"),
				t_removeEventListener(S, C, x),
				t_addEventListener(S, C, x, !0),
				t_removeEventListener(S, "change", x),
				t_addEventListener(S, "change", x, !0);
		}
		...
		for (
			...
			if (c && c.obj) {
				...
				if (f && f instanceof Element)
					t_forms__showInputErrors(o, f.closest(s), c);
	...
	(window.tildaForm.successEnd = function (t, e, r) {
		var o = t_forms__getElement(t);
		t_addClass(o, "js-send-form-success"),
			t_forms__updateAllSystemVariables(o),
					...
					if (4 === xhr.readyState)
						if (xhr.status >= 200 && xhr.status < 400) {
								...
								if ("{" == o.substring(0, 1)) {
									...
									if ("object" == typeof n)
										if (n && n.message && "OK" !== n.message)
											...
											var a,
												i = (a = t_forms__getErrorContainers(
													form,
													"Unknown error. Please, try again later."
												)).errorBoxes,
												s = a.allError;
							...
							t_removeClass(btnSubmit, "t-btn_sending"),
								(btnSubmit.tildaSendingStatus = "0");
							var a,
								i = (a = t_forms__getErrorContainers(form, "")).errorBoxes,
								s = a.allError,
								o = xhr.responseText;
							Array.prototype.forEach.call(s, function (t) {
								o
...
function t_forms__onSuccess(t) {
	var e = t_forms__getElement(t),
		...
		c = e.querySelector(".t-form__successbox"),
		m = c ? c.getBoundingClientRect().top + window.pageYOffset : 0,
		f = 0,
	...
	if (
		...
		var x = r.querySelector(".t" + o + "__btn_prev"),
...
function t_forms__handleSuccess(form, options) {
	...
	callback && "function" == typeof window[callback]
		? "undefined" != typeof jQuery
	...
	for (
		...
		if (
			...
			if ("select-multiple" !== r[a].type) {
				...
				for (var l = r[a].options, d = 0; d < l.length; d++)
					l[d].selected && e.push({ name: l[d].name, value: l[d].value });
	return e;
}
